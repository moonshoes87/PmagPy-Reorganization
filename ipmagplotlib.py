...
from numpy import polyfit, ceil
globals = 0
graphmenu = 0
global version_num
...
def plot_init(fignum,w,h):
    ...
    if not isServer:
        ...
        #plt.get_current_fig_manager().window.wm_geometry('+%d+%d' % (fig_x_pos,fig_y_pos)) # this only works with matplotlib.use('TKAgg')
...
def plotPTS(fignum,PTs,x,y):
    for pt in PTs:
        plt.scatter(pt[x],pt[y],marker=pt['marker'],c=pt['color'],s=pt['size'])
...
#          print polynomial
    ...
    if 'xlab' in kwargs.keys():plt.xlabel(kwargs['xlab'])
    if 'ylab' in kwargs.keys():plt.ylabel(kwargs['ylab'])
    if 'title' in kwargs.keys():plt.title(kwargs['title'])
    if 'xmin' in kwargs.keys(): plt.axis([kwargs['xmin'],kwargs['xmax'],kwargs['ymin'],kwargs['ymax']])
...
def plotQQunf(fignum,D,title):
    ...
    notestr='Mu: '+'%7.3f'%(Mu)
    plt.text(.1*bounds[1],.85*bounds[3],notestr)
    if Mu > 1.347:
...
#
    plt.figure(num=fignum)
    plt.clf()
    if not isServer: plt.figtext(.02,.01,version_num)
    plt.axis("off")
...
#        plt.scatter(X_down,Y_down,marker='s',c='r')
        plt.scatter(X_down,Y_down,marker='o',c='c')
        if globals !=0:
            globals.DIlist  = X_down
            globals.DIlisty = Y_down
    if len(X_up)>0:
#        plt.scatter(X_up,Y_up,marker='s',c='w')
        plt.scatter(X_up,Y_up,marker='o',c='w')
        if globals !=0:
            globals.DIlist  = X_up
...
#
    ...
    if 'edgecolor' not in sym.keys():
        sym['edgecolor']='k'
    if len(X_down)>0:
        plt.scatter(X_down,Y_down,marker=sym['lower'][0],c=sym['lower'][1],s=size,edgecolor=sym['edgecolor'])
        if globals !=0:
            globals.DIlist  = X_down
            globals.DIlisty = Y_down
    if len(X_up)>0:
        plt.scatter(X_up,Y_up,marker=sym['upper'][0],c=sym['upper'][1],s=size,edgecolor=sym['edgecolor'])
        if globals !=0:
            globals.DIlist  = X_up
            globals.DIlisty = Y_up
...
def plotZED(ZED,datablock,angle,s,units):
    ...
    plt.plot(AngleX,AngleY,'r-') # Draw a line for Zijderveld horizontal axis
    if AngleX[-1]==0:AngleX[-1]=0.01
    plt.text(AngleX[-1]+(AngleX[-1]/abs(AngleX[-1]))*.1,AngleY[-1]+(AngleY[-1]/abs(AngleY[-1]))*.1,'X')
    norm=1
    if units=="U": norm=0
...
# now plot stuff
    if len(x) ==0:
        ...
        return
    try:
        if len(x_zi)>0:plt.scatter(x_zi,y_zi,marker='o',c='r',edgecolors="none" ) # zero field-infield
        if len(x_iz)>0:plt.scatter(x_iz,y_iz,marker='s',c='b',faceted="True" ) # infield-zerofield
    except:
        if len(x_zi)>0:plt.scatter(x_zi,y_zi,marker='o',c='r') # zero field-infield
        if len(x_iz)>0:plt.scatter(x_iz,y_iz,marker='s',c='b') # infield-zerofield
    plt.plot(x,y,'r' )
    if globals !=0:
        globals.MTlist = x
        globals.MTlisty=y
...
#
    ...
    if units=="K":plt.xlabel("Temperature (C)")
    if units=="J":plt.xlabel("Microwave Energy (J)")
...
def saveP(Figs,filenames):
    for key in Figs.keys():
        try:
            ...
            plt.savefig(filenames[key].replace('/','-'))
            if verbose:
...
#
            ...
            X.append(XY[0])
            Y.append(XY[1])
        plt.scatter(X,Y,s=symsize,marker=symb[VEC],c=col[VEC],edgecolors='none')
    plt.axis("equal")
#
...
#            for i in range(3): elli[i]=-elli[i]
    ...
    if plot==1:
        if X_ell!=[]:plt.plot(X_ell,Y_ell,col)
        if X_up!=[]:plt.plot(X_up,Y_up,'g-')
    else:
        return PTS
#
#
...
def plotVs(fignum,Xs,c,ls):
    ...
    for xv in  Xs:
        bounds=plt.axis()
        plt.axvline(x=xv,ymin=bounds[2],ymax=bounds[3],linewidth=1,color=c,linestyle=ls)
...
# close up loop
    ...
    Mslop=Mfix[2:Nint+2]
    polyU=plt.polyfit(Bslop,Mslop,1) # best fit line to high field points
    Bslop=B[kmin:kmin+(Nint+1)] # adjust slope with first 30 points of ascending branch
    Mslop=Mfix[kmin:kmin+(Nint+1)]
    polyL=plt.polyfit(Bslop,Mslop,1) # best fit line to high field points
...
# find Bc (x intercept), interpolate between two bounding points
    ...
    try:
       poly=plt.polyfit(Bz,Mz,1) # best fit line through two bounding points
       Bc=-poly[1]/poly[0] # x intercept
       poly=plt.polyfit(Baz,Maz,1) # best fit line through two bounding points
...
def plotzfcfc(MT,e):
    ...
    if init!=0:
        ...
        if len(ZFCM)>2: plt.plot(ZFCT,ZFCM,'bo')
        if len(FCM)>2: plt.plot(FCT,FCM,'ro')
        plt.title(e)
#
def plotltc(LTC_CM,LTC_CT,LTC_WM,LTC_WT,e):
    ...
    if init!=0:
        ...
        plt.xlabel('Temperature (K)')
        plt.ylabel('Magnetization (Am^2/kg)')
        if len(LTC_CM)>2: plt.plot(LTC_CT,LTC_CM,'bo')
        if len(LTC_WM)>2: plt.plot(LTC_WT,LTC_WM,'ro')
        plt.title(e)
    ...
    if iboot==1:
        ...
        if plt==1:
            ...
            if comp==1: # do eigenvector of choice
                ...
                plt.axvline(x=vbounds[0],linewidth=1,color='r',linestyle='--')
                plt.axvline(x=vbounds[1],linewidth=1,color='r',linestyle='--')
                #plotVs(ANIS['vxcdf'],vbounds,'r','--')
                #plotVs(ANIS['vxcdf'],[Ccart[0]],'r','-')
                plt.axvline(x=Ccart[0][0],linewidth=1,color='r',linestyle='-')
                ...
                plt.axvline(x=vbounds[0],linewidth=1,color='b',linestyle='--')
                plt.axvline(x=vbounds[1],linewidth=1,color='b',linestyle='--')
                plt.axvline(x=Ccart[0][1],linewidth=1,color='b',linestyle='-')
                ...
                plt.axvline(x=vbounds[0],linewidth=1,color='k',linestyle='--')
                plt.axvline(x=vbounds[1],linewidth=1,color='k',linestyle='--')
                plt.axvline(x=Ccart[0][2],linewidth=1,color='k',linestyle='-')
...
def plotPIE(fig,fracs,labels,title):
    ...
    plt.figure(num=fig)
    plt.pie(fracs,labels=labels,colors=('r','y','b','g','m','c','w'),explode=explode)
    plt.title(title)
#
...
#
    ...
    if new==1:
        ...
        plt.scatter(x,y,marker='^',s=80,c='g')
    plt.title(s)
#
# plot the ellipse
#
        ...
        while k<len(lats)-1:
            if  lats[k]<=90: # part of string
                ...
                if y<1e20:Y.append(y) # exclude off the map points
       	        if prn_name==1: T.append(plt.text(x+5000,y-5000,names[k]))
                k+=1
...
def plotEQcont(fignum,DIblock):
    ...
    xlist = plt.linspace(a1,a2,int(ceil(num)))
    ylist=plt.linspace(b1,b2,int(ceil(num)))
    ...
    for i in range(0,int(ceil(num))**2):
    ...
    count=0
    dotspercircle=0.
    for j in range(0,int(ceil(num))):
    		for i in range(0,int(ceil(num))):
    					...
    					count+=1
    					dotspercircle=0.
    im = plt.imshow(Z, interpolation='bilinear', origin='lower', cmap=plt.cm.hot,extent=(-1.,1.,-1.,1.))
    plt.colorbar()
    x,y=[],[]
    # Draws the border
